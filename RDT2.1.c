...
rtp_packet_t make_pkt(int seqnum,char *data,char checksum)
{
    char payload[5];
    int i;
    for(i=0;i<5;i++){
...
rtp_packet_t rdt_rcv(rtp_packet_t packet)
{
    ...
    case wait0:
        ...
        if(notcorrupt(packet)&&packet.seqnum==0)
        {
            ...
            rtp_packet_t sndpkt = make_pkt(0,app_ack,complemento_checksum(app_ack));
        ...
        {
            printf("\n NACK \n");
            rtp_packet_t sndpkt = make_pkt(0,app_nack,complemento_checksum(app_nack));
            estado_receiver = wait0;
            return sndpkt;
        }
        break;
    case wait1:
        printf("\nestado receiver:wait1\n");
        if(notcorrupt(packet) && packet.seqnum==1)
        {
            app_layer_B(packet.payload);//deliver_data to receiver application_layer.
            rtp_packet_t sndpkt = make_pkt(1,app_ack,complemento_checksum(app_ack));
            estado_receiver = wait0;
            return sndpkt;
        }
        ...
        {
            printf("\n NACK \n");
            rtp_packet_t sndpkt = make_pkt(1,app_nack,complemento_checksum(app_nack));
            estado_receiver = wait1;
            return sndpkt;
        }
        break;
...
int isAck(rtp_packet_t packet)
{
    ...
    else if(packet.payload[0]=='i'&&packet.payload[1]=='s'&&packet.payload[2]=='n'&&packet.payload[3]=='a'&&packet.payload[4]=='k')
    {
        return 0;
    }
    return 0;
...
void rdt_send(char *data)
{
    ...
    do{
        ...
        {
        case send_and_wait0:
            if(packet_loss_sender==0&&packet_loss_receiver==0){
            printf("\nSender estado send:0\n");
            sndpkt = make_pkt(0,data,complemento_checksum(data));
            ...
            {
                //ACK
                if(notcorrupt(rcvpkt) && isAck(rcvpkt)==1)
                {
        ...
        case send_and_wait1:
            ...
            sndpkt = make_pkt(1,data,complemento_checksum(data));
    ...
    }while(packet_loss_sender==1||packet_loss_receiver==1);
}
